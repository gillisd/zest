...
test_refute_equal_with_spaces() {
  run refute_equal "hello world" "hello  world"
  assert_equal 0 $rc
}

test_refute_equal_with_spaces_match() {
  run refute_equal "hello world" "hello world"
  assert_equal 1 $rc
}

test_refute_equal_numeric_strings() {
  run refute_equal 42 043
  assert_equal 0 $rc
}

test_refute_equal_numeric_strings_match() {
  run refute_equal 42 42
  assert_equal 1 $rc
}

test_refute_equal_trailing_newline_mismatch() {
  run refute_equal $'foo\n' "foo"
  assert_equal 0 $rc
}

test_refute_equal_trailing_newline_match() {
  run refute_equal $'foo\n' $'foo\n'
  assert_equal 1 $rc
}

test_refute_equal_embedded_newline_mismatch() {
  run refute_equal $'foo\nbar' $'foobar'
  assert_equal 0 $rc
}

test_refute_equal_embedded_newlines_match() {
  run refute_equal $'foo\nbar' $'foo\nbar'
  assert_equal 1 $rc
}

test_refute_equal_crlf_vs_lf() {
  run refute_equal $'foo\r\n' $'foo\n'
  assert_equal 0 $rc
}

test_refute_equal_newline_only_vs_empty() {
  run refute_equal $'\n' ""
  assert_equal 0 $rc
}
